...
AND NOT EXISTS (
  SELECT 1 FROM soins WHERE nom = 'Box braids mi-longues'
)
...
FROM types_soins WHERE nom = 'Tresses & Braids'
AND NOT EXISTS (
  SELECT 1 FROM soins WHERE nom = 'Box braids longues'
)
...
FROM types_soins WHERE nom = 'Tresses & Braids'
AND NOT EXISTS (
  SELECT 1 FROM soins WHERE nom = 'Vanilles / Twists'
)
...
AND NOT EXISTS (
  SELECT 1 FROM soins WHERE nom = 'Soin hydratant profond'
)
...
FROM types_soins WHERE nom = 'Soins & Traitements'
AND NOT EXISTS (
  SELECT 1 FROM soins WHERE nom = 'Shampoing + Brushing'
)
...
FROM types_soins WHERE nom = 'Soins & Traitements'
AND NOT EXISTS (
  SELECT 1 FROM soins WHERE nom = 'Défrisage'
)
...
AND NOT EXISTS (
  SELECT 1 FROM soins WHERE nom = 'Tissage mi-long'
)
...
FROM types_soins WHERE nom = 'Tissage & Perruques'
AND NOT EXISTS (
  SELECT 1 FROM soins WHERE nom = 'Pose de perruque'
)
...
AND NOT EXISTS (
  SELECT 1 FROM soins WHERE nom = 'Coiffure mariage'
)
...
FROM types_soins WHERE nom = 'Coiffures Événement'
AND NOT EXISTS (
  SELECT 1 FROM soins WHERE nom = 'Coiffure soirée'
);